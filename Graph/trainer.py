...
import utils
from Graph.data.data_helper import DataHelper
from Graph.optims import Optim
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.logging = "C:\Users\Omkar Kashyap\OneDrive\Desktop\MulQG\output\GraphSage" + utils.format_time(
            time.localtime())
        self.log_path = utils.logging_csv(self.logging + 'record.csv')
        self.log_csv = utils.logging_csv(self.logging + 'log.txt')
        ...
        self.model = GraphSage(config=self.args, embeddings = embedding)
        ...
        self.model = torch.nn.DataParallel(self.model, device_ids=[0],dim=-1)
    ...
    #     log_dir= 'C:\\Users\Omkar Kashyap\OneDrive\Desktop\MulQG\output\GraphPointerGenerator'
    ...
    def print_model_info(self):
        ...
        for param in self.model.parameters():
            if param.requires_grad:
                param_count += param.view(-1).size()[0]
        #'total number of parameters need to be optimized: %d\n\n' % param_count
